...
- 登場するパッケージ
  ...
  - python-requests-2.10.0-1.el7ost.noarch
  - python-nova-13.1.2-9.el7ost.noarch
...
長いけど要はlibvirtドライバのswap_volume()を呼んでいる。
...
libvirtドライバの中でさらに\_swap\_volume()を呼ぶ。
...
最終的にlibvirt-pythonのblockRebase()を呼び出す。

libvirt-pythonのコードは/usr/lib64/python2.7/site-packagesがトップディレクトリ。
...
libvirt APIのvirDomainBlockRebase()を呼んでいるだけ。

# ここからlibvirt

- src/libvirt-domain.c [libvirt]

virtHypervisorDriver(この場合はqemuHypervisorDriver)のdomainBlockRebase()メソッドを呼ぶ。
                     ...
                     unsigned int flags)
{
    ...
    if (conn->driver->domainBlockRebase) {
        int ret;
        ret = conn->driver->domainBlockRebase(dom, disk, base, bandwidth, /* XXX */
                                              flags);
        if (ret < 0)
            goto error;
        return ret;
                      ...
                      unsigned long bandwidth, unsigned int flags)
{
    ...
    flags &= (VIR_DOMAIN_BLOCK_REBASE_SHALLOW |
              VIR_DOMAIN_BLOCK_REBASE_REUSE_EXT);
    ret = qemuDomainBlockCopyCommon(vm, dom->conn, path, dest, /* XXX */
                          ...
                          bool keepParentLabel)
{
    ...
    ret = qemuMonitorDriveMirror(priv->mon, device, mirror->path, format, /* XXX */
...
長いけど、qemuMonitorDriveMirror()が実際のミラー処理。
...
qemuの世界のdrive-mirror QMPは[こちら](https://github.com/orimanabu/CodeReading/blob/master/Qemu/drive-mirror.md)に移動。
